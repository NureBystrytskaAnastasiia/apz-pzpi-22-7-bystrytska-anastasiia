...
exports.getMedicinesWithStorageIssues = async (req, res) => {
  try {
    const medicines = await Medicine.find().populate('storageRoomId', 'name temperature humidity');
    
    const results = medicines.map(medicine => {
      const roomTemp = medicine.storageRoomId?.temperature || null;
      const roomHumidity = medicine.storageRoomId?.humidity || null;
      
      // Перевірка умов зберігання
      const { isTempValid, isHumidityValid } = checkStorageConditions(medicine, roomTemp, roomHumidity);
      
      // Визначення статусу
      let storageStatus;
      if (roomTemp === null || roomHumidity === null) {
        storageStatus = 'Дані про умови відсутні';
      } else {
        storageStatus = isTempValid && isHumidityValid 
          ? 'Оптимальні умови' 
          : 'Поза межами норми';
      }
      ...
      return {
        id: medicine._id,
        name: medicine.name,
        storageRoom: medicine.storageRoomId?.name || 'Невідомо',
        currentTemperature: roomTemp,
        currentHumidity: roomHumidity,
        temperatureRange: medicine.temperatureRange,
    ...
    console.error('Помилка при отриманні ліків з проблемами зберігання:', error);
    res.status(500).json({ 
